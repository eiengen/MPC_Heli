...
#define NCONTROLINPUTS  120  /*2	*/				/**< Number of unknowns to optimize. */
#define MAXITER         400	    				/**< Maximum number of iterations. */
...
static void mdlStart(SimStruct *S)
{
    ...
    /* my options: start */
    /*problemOptions.maxIter = 100;
    problemOptions.maxCpuTime = 0.1;*/
    /*problemOptions.enableRegularisation = BT_TRUE;
    problemOptions.printLevel = PL_LOW;
    problemOptions.enableEqualities = BT_TRUE;*/
    /* my options: ends */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	if ( problem->getCount() == 0 )
	{
        ...
        status = problem->init( H,g,A,lb,ub,lbA,ubA, nWSR,CPUTIME);/*,my_ini );*/
	...
	else
	{
		...
		if ( ( status != SUCCESSFUL_RETURN ) && ( status != RET_MAX_NWSR_REACHED ) )
		{
			...
			status = problem->init( H,g,A,lb,ub,lbA,ubA, nWSR_retry,CPUTIME);/*,my_ini );*/
            nWSR += nWSR_retry;
		}
